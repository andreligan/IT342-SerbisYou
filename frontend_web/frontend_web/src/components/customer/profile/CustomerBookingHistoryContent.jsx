...
const CustomerBookingHistoryContent = () => {
  ...
  const [reviewedBookings, setReviewedBookings] = useState({}); // Track which bookings have been reviewed
    ...
    const fetchBookingHistory = async () => {
      try {
        ...
        const reviewStatusPromises = response.data
          ...
          .map(async (booking) => {
            try {
              const reviewCheckResponse = await axios.get("/api/reviews/can-review", {
                params: {
